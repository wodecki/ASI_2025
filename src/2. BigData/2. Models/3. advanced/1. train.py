...
logger.info("\nNote: AutoGluon will train multiple models and create an ensemble.")
logger.info("For time series, we use time-based splits (not random cross-validation)")
...
logger.info("\n" + "="*60)
logger.info("Model Performance Comparison (Leaderboard):")
logger.info("="*60)
logger.info(f"{'Model Name':<30} {'Score':<15} {'Fit Time (s)':<15}")
logger.info("-"*60)
...
for idx, row in leaderboard.iterrows():
    ...
    if isinstance(fit_time, (int, float)):
        fit_time_str = f"{fit_time:.2f}"
    else:
        fit_time_str = str(fit_time)

    logger.info(f"{model_name:<30} {score_str:<15} {fit_time_str:<15}")

logger.info("="*60)
logger.info(f"Best model: {best_model}")
logger.info(f"Evaluation metric: {eval_metric} (lower is better for MASE)")
logger.info("="*60)
...
metadata = {
    ...
    "notes": {
        "cross_validation": "Time series use time-based splits, not random CV",
        "ensemble_strategy": "WeightedEnsemble combines predictions from all models"
    }
}
...
logger.info("\nNext steps:")
logger.info("1. Run '3. evaluate.py' to generate detailed evaluation report")
logger.info("2. Run '2. predict.py' to make predictions")
logger.info("="*60)
